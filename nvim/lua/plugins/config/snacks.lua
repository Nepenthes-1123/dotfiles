...
setup("snacks", function(Snacks)
	Snacks.setup({
		...
		scroll = {
			...
			-- スクロールバーの表示
			filter = function(buf)
				return vim.g.smooth_scroll_enabled ~= false and vim.bo[buf].buftype ~= "terminal"
			end,
		},
		...
		words = {
			enabled = true, -- vim-illuminate を使用しているため無効
		},
	...
	vim.api.nvim_create_autocmd("LspProgress", {
		...
		callback = function(ev)
			...
			if value.kind == "begin" then
				-- プログレス開始: 通知を作成
				progress_handles[key] =
					Snacks.notify.info((value.message or "") ~= "" and value.message or (value.title or "Loading..."), {
						id = key,
						title = client.name,
						opts = function(notif)
							notif.icon = "◐ "
	...
	map("n", "<Leader>fg", function()
		Snacks.picker.grep()
	end, vim.tbl_extend("force", opts, { desc = "File Grep" }))
	...
	end, vim.tbl_extend("force", opts, { desc = "GitHub issues" }))

	-- PR 一覧 (VSCode: GitHub Pull Requests サイドバーの "Pull Requests" セクション)
	map("n", "<Leader>gpl", function()
		Snacks.picker.gh_pr()
	end, vim.tbl_extend("force", opts, { desc = "GitHub PRs" }))
	vim.keymap.set("n", "<leader>gpb", function()
		Snacks.picker.gh_prs({
			-- "current" を指定すると、現在のブランチに関連する PR に絞り込まれます
			filter = {
				branch = true,
			},
		})
	end, { desc = "GitHub PRs for current branch" })
	...
	end, vim.tbl_extend("force", opts, { desc = "Dismiss all notifications" }))
