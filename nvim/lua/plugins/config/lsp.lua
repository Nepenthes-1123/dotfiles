...
setup('mason-lspconfig', function(m)
    m.setup({
        ensure_installed = {
            'pyright', 'ruff', 'clangd', 'lua_ls', 'vue_ls',
            'ts_ls', 'texlab', 'marksman', 'jsonls', 'dockerls', 'docker_compose_language_service'
        },
...
setup('mason-tool-installer', function(m)
    m.setup({
        ensure_installed = {
            'ruff', 'stylua', 'prettier', 'eslint_d',
            'clang-format', 'markdownlint',
        },
        auto_update      = false,
        run_on_start     = true,
        start_delay      = 1500,
...
setup('conform', function(m)
    m.setup({
        ...
        formatters = {
            ...
            prettier     = { prepend_args = { '--trailing-comma', 'none' } },
        },
